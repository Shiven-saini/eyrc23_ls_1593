...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  jacobian_matrices = {};
  ################## ADD YOUR CODE HERE ######################
  equation = [x1_dot; x2_dot];
  j = jacobian(equation, [x1, x2]);
  j1 = subs(j, {x1, x2}, [x_1(1), x_2(1)]);
  j2 = subs(j, {x1, x2}, [x_1(2), x_2(2)]);
  j3 = subs(j, {x1, x2}, [x_1(3), x_2(3)]);
  j1 = double(j1);
  j2 = double(j2);
  j3 = double(j3);
  jacobian_matrices = {j1, j2, j3};
  ############################################################  
endfunction
